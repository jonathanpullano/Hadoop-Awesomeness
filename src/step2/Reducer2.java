...
public class Reducer2 extends
    ...
    private final static Logger LOGGER = Logger.getLogger(Reducer2.class
            .getName());
    ...
    protected void reduce(
        ...
        if (Constants.DEBUG)
            LOGGER.info("Reducer2 - reduce called");

        final Iterator<Tuple> iter = tuples.iterator();
        while (iter.hasNext()) {
            final Tuple t1 = new Tuple(iter.next());
            set.union(t1.getFirst(), t1.getSecond());
            inputs.add(t1.getFirst());
            if (Constants.DEBUG)
                LOGGER.info("Reducer2 - pass1 looped");
        }

        for (final Integer pVal : inputs) {
            context.write(new IntWritable(pVal),
                    new IntWritable(set.find(pVal)));
            if (Constants.DEBUG)
                LOGGER.info("Reducer2 - pass2 looped");
        }
    }
}
