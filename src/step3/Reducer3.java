...
public class Reducer3 extends
	...
	protected void reduce(
		...
		/*
		 * Iterator<Tuple> iter = values.iterator(); int prevP = -1;
		 * while(iter.hasNext()) { Tuple curTup = iter.next(); int curP =
		 * curTup.getFirst(); if(curP == prevP) boundaryMemory.put(curP,
		 * curTup.getSecond()); else pointMemory.add(curP); prevP = curP; }
		 */
		...
		for (int p = minP; p <= maxP; p++) {
			...
			if (((p > height) && pointMemory.contains(p - height))
					|| boundaryMemory.containsKey(p - height)) {
				set.union(p, p - height);
			}

			if ((((p % height) != 1) && pointMemory.contains(p - 1))
					|| boundaryMemory.containsKey(p - 1)) {
				set.union(p, p - 1);
			}
