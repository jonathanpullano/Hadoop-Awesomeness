...
public class Reducer5 extends Reducer<IntWritable, Tuple, Text, Text> {
	...
	protected void reduce(final IntWritable key, final Iterable<Tuple> values,
		...
		/*
		 * To run the extra credit, uncomment the following lines and run
		 * ExtraCreditBase.java
		 */
		/*
		 * final Writer out = new OutputStreamWriter(new FileOutputStream(
		 * "stats.csv", true)); final DecimalFormat df = new
		 * DecimalFormat("0.00"); final String desired_density =
		 * df.format(Constants.desiredDensity); try { out.write(desired_density
		 * + ", " + total_vertices + ", " + total_edges + ", " +
		 * total_components + ", " + max_component_size + ", " +
